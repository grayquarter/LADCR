...
try {
    ...
    var sql = "select b1_per_id1, b1_per_id2, b1_per_id3 from bpermit_detail where b1_priority = 'Ineligible for Processing' and serv_prov_code = 'LADCR' ";
    ...
    for (var i in r) {
        ...
        if (capId) {
            aa.print("=== record " + capId.getCustomID());
            deactivateAllTasksByCap(capId);
            activateTaskByCap("Close Out", capId);
        } else {
            aa.print("=== record not found for " + r[i].b1_per_id1 + "-" + r[i].b1_per_id2 + "-" + r[i].b1_per_id3);
        }
    }
...
function deactivateAllTasksByCap(capId) 
{
    ...
    for (var i in wfObj) 
    {
		...
		aa.print("Deactivating Workflow Task: " + fTask.getTaskDescription());
    }
}
