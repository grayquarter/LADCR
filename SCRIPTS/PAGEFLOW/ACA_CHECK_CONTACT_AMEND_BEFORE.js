/*------------------------------------------------------------------------------------------------------/
| Program : ACA_CHECK_CONTACT_AMEND_BEFORE.js
...
try {
    ...
    if (badContactArray.length > 0) {
        cancel = true;
        showMessage = true;
        comment("The following contacts cannot be added with this type of amendment: " + badContactArray.join(",") + ".");
    }
} catch (err) {
    logDebug(err);
}
