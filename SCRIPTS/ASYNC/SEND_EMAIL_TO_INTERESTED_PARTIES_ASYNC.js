		...
		if (peopleArr && peopleArr.length > 0) {
			for (cnt in peopleArr) {
				...
				if (notDuplicate) {
					...
					aa.print("Email Sent: " + aa.document.sendEmailAndSaveAsDocument(pMailFrom, attrEmail, "", pEmailTemplate, vEParamsToSend, capId4Email, attachDocs ? attachedDocumentFiles : null).getSuccess());
