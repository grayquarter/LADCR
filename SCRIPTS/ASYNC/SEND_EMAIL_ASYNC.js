	...
	if (sendEmailToAddresses && sendEmailToAddresses != '') {
		...
		for (var i = 0; i < arEmails.length; i++) {
			...
			if (!exists(thisEmail.toUpperCase(), sentTo)) {
				...
				logDebug("Email Sent: " + aa.document.sendEmailAndSaveAsDocument(mailFrom, thisEmail, "", emailTemplate, vEParams, capId4Email, attachDocs ? attachedDocumentFiles : null).getSuccess());
